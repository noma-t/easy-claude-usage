...
GitHub Actionsのビルドが完了すると、GitHub Releasesにドラフトが作成されます。内容（アセット一覧: NSISインストーラー・ポータブルexe・`latest.json`）を確認し、問題なければPublishしてください。Publish後、既存クライアントが自動アップデート確認（起動時・トレイメニューの操作）で新バージョンを検知できるようになります。

## アップデートの検知・適用

アップデートの検知と適用はすべてタスクトレイ側（Rustバックエンド）で完結しており、フライアウトUIは一切関与しません。

- 起動時に自動でバックグラウンドチェックを行います。
- 新しいバージョンが見つかると、タスクトレイのアイコン右上に丸いバッジが表示されます。
- トレイの右クリックメニュー内の項目は状況に応じて1つに統合されています。
  - 更新が見つかっていない時：「アップデートを確認」（クリックで手動チェック）
  - 更新が見つかっている時：「アップデート」（クリックでダウンロード → インストール → 自動再起動）
